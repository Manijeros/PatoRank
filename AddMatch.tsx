...
function AddMatch({ players, navigation }: AddMatchProps & { navigation: any }) {
  const [ selectedPositions, setSelectedPositions ] = useState({} as { [index: string]: number })
  ...
  return (<View style={{ flex: 1 }}>
    ...
    <SafeAreaView style={{ alignContent: 'flex-end' }}>
      <TouchableHighlight
        ...
        onPress={ enableSend ? () => send(selectedPositions, navigation) : undefined }>
        <Text style={{
          color: 'white',
          textAlign: 'center',
          fontWeight: 'bold',
...
function checkData(players: PlayerData[], selectedPositions: Selection) {
  ...
  R.keys(selectedPositions).forEach(key => used[selectedPositions[key] - 1] += 1, selectedPositions)
  if (used[0] != 1) {
    return false
  }
  if (R.reduce((r, n) => n > 0 && r == 0 ? -1 : n == 0 ? 0 : r, 1, used) == -1) {
    return false
  }
  return true
}
function send(selectedPositions: Selection, navigation: any) {
  ...
  (R.keys(selectedPositions) as string[]).forEach(key => {
    ...
    switch (position) {
      case 1: match.first = key; break
      case 2: match.second.push(key); break
      case 3: match.third.push(key); break
      case 4: match.fourth.push(key); break
    }
  })
